...
while IFS="|" read commit message; do
    ...
    countTotal=$(git diff ${commit}^ $commit -- analyzed_stats | grep -A 1 "Total Masternodes:" | tail -1)
    countEnabled=$(git show ${commit}:analyzed_stats | awk '/ENABLED/ {print $1}')
    countEnabled=$(echo $countEnabled | tr -d -c "0-9") # this ensures we only take the absolute value of the count
    echo "$timestamp $countEnabled"
done > /tmp/data.txt
...
gnuplot -p -e "
    ...
    set format x '%d/%m\n%H:%M';
    set xlabel 'Time';
    set ylabel 'ENABLED Masternode count';
    plot '/tmp/plot_data.txt' using 1:3 with linespoints title 'ENABLED Masternode count over time';
"
